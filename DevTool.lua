-- Copyright (C) 2025 Quravat Studios. All rights reserved.
-- Author: Ghali Amli
-- This script is protected by copyright law and may not be redistributed, modified, or used for commercial purposes without explicit permission from Quravat Studios.
-- For permission or inquiries, contact: contact.quravat@gmail.com
-- File: DevTool.lua
...
RegisterCommand("devplayers", function()
    showPlayers = not showPlayers
    if showPlayers then
        Citizen.CreateThread(function()
            while showPlayers do
                Citizen.Wait(0)
                for _, playerId in ipairs(GetPlayers()) do
                    local playerPed = GetPlayerPed(playerId)
                    local playerName = GetPlayerName(playerId)
                    local playerSessionID = GetPlayerServerId(playerId)
                    local coords = GetEntityCoords(playerPed)
                    local health = GetEntityHealth(playerPed)
                    local armor = GetPedArmour(playerPed)
                    DrawText3D(coords.x, coords.y, coords.z + 1.0, "Name: " .. playerName .. "\nSession ID: " .. playerSessionID .. "\nHealth: " .. health .. "\nArmor: " .. armor .. "\nCoords: " .. coords.x .. ", " .. coords.y .. ", " .. coords.z)
                end
            end
        end)
    end
...
local showPeds = false

RegisterCommand("devpeds", function()
    showPeds = not showPeds
    if showPeds then
        Citizen.CreateThread(function()
            while showPeds do
                Citizen.Wait(0)
